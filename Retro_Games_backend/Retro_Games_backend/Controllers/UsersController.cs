...
public class UsersController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<LeaderboardEntry>>> GetLeaderboard([FromBody] GameRequestModel request)
    {
        ...
        if (string.IsNullOrEmpty(request.GameName)) return BadRequest("GameName is required");

        //obtinem informatii depsre joc
        var game = await GetGameNameByNameAsync(request.GameName);
        if (game == null) return NotFound($"Game '{request.GameName}' not found.");
    ...
    public async Task<ActionResult<int>>
        GetHighScore([FromBody] GameRequestModel request) //ne citeste nume jocului din body
    {
        //extragem UserId fin token ul JWT
        var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier);
        if (userIdClaim == null ||
            !int.TryParse(userIdClaim.Value,
                out var userId)) return Unauthorized(); //daca id ul utilizatorului nu poate fi exstras sau e invalid
    ...
    public async Task<IActionResult> UpdateHighScore([FromBody] ScoreUpdateModel model)
    {
        ...
        if (userIdClaim == null ||
            !int.TryParse(userIdClaim.Value,
                out var userId)) return Unauthorized(); //daca id ul utilizatorului nu poate fi extras sau e invalid

        //obtinem informatiile despre joc
        var game = await GetGameNameByNameAsync(model.GameName);
        if (game == null) return NotFound($"Game '{model.GameName}' not found.");
