...
@Component({
  selector: 'app-brickbreakertwo3',
  imports: [CommonModule],
  templateUrl: './brickbreakertwo3.html',
  styleUrl: './brickbreakertwo3.css'
})

export class Brickbreakertwo3 {
  ...
  time = 200; //folosita pentru a reporni intervalul
  ...
  PaddlePosition: { x: number, y: number } = {x: 4, y: 19}; //pozitia initiala a paddle ului de jos
  PaddleTwoPosition: { x: number, y: number } = {x: 4, y: 0}; //pozitia initiala a paddle ului de sus
  BricksPosition: { x: number, y: number } = {x: 0, y: 7}; //pozitia initiala a brick urilor
  BallPosition: { x: number, y: number } = {x: 5, y: 18}; //pozitia initiala a bilei
  ...
  ngOnInit() {
    ...
    //initializam arena pentru vieti cu 1 linie si 4 coloane, puse pe 0
    this.lifeArena = [];
    for (let i = 0; i < 1; i++) {
  ...
  checkBallCollision() {
    ...
    {
      if(
        this.BallPosition.x === this.PaddlePosition.x - 1 || //adaugam si cazul in care bila loveste coltul paddle ului
        this.BallPosition.x === this.PaddlePosition.x ||
        this.BallPosition.x === this.PaddlePosition.x + 1 ||
        this.BallPosition.x === this.PaddlePosition.x + 2 //adaugam si cazul in care bila loveste coltul paddle ului
    ...
    {
      if(
        this.BallPosition.x === this.PaddleTwoPosition.x - 1 || //adaugam si cazul in care bila loveste coltul paddle ului
        this.BallPosition.x === this.PaddleTwoPosition.x ||
        this.BallPosition.x === this.PaddleTwoPosition.x + 1 ||
        this.BallPosition.x === this.PaddleTwoPosition.x + 2 //adaugam si cazul in care bila loveste coltul paddle ului
      ) //verificam conexiunea cu paddle ul pe axa x
      {
  ...
  resetGameRound() {
    ...
    this.BallGoingUp = true;
    this.BallPosition.x = 5, this.BallPosition.y = 18; //reinitializam pozitia bilei
    this.PaddlePosition.x = 4, this.PaddlePosition.y = 19; //reinitializam pozitia paddle ului de jos
    this.PaddleTwoPosition.x = 4, this.PaddleTwoPosition.y = 0; //reinitializam pozitia paddle ului de sus
  ...
  movePaddleLeft() {
    ...
    this.checkBallCollision(); //verificam coliziunile bilei cu peretii si paddle urile
  }

  //functie pentru a muta paddle urile la dreapta
  movePaddleRight() {
    if(this.PaddlePosition.x + 1 < 9)
    {
      this.PaddlePosition.x++;
      if(this.BallPosition.y + 1 === this.PaddlePosition.y && 
        (this.BallPosition.x === this.PaddlePosition.x - 1 || //adaugam si cazul in care bila loveste coltul paddle ului
    ...
    if(this.PaddleTwoPosition.x + 1 < 9)
    {
      this.PaddleTwoPosition.x++;
      if(this.BallPosition.y - 1 === this.PaddleTwoPosition.y && 
        (this.BallPosition.x === this.PaddleTwoPosition.x - 1 || //adaugam si cazul in care bila loveste coltul paddle ului
  ...
  startGame() {
    ...
    this.PaddlePosition.x = 4, this.PaddlePosition.y = 19 //pozitia initiala a paddle ului de jos
    this.PaddleTwoPosition.x = 4, this.PaddleTwoPosition.y = 0; //pozitia initiala a paddle ului de sus
    this.BricksPosition.x = 0, this.BricksPosition.y = 7; //pozitia initiala a brick urilor
    this.BallPosition.x = 5, this.BallPosition.y = 18; //pozitia initiala a bilei
    ...
    this.time = 200; //resetam timpul intervalului
  ...
  //functie pentru a pune jocul pe pauza sa a l relua
  togglePause() {
  ...
  moveBallFaster() {
    ...
    this.points += 3; //marim cu 3 puncte atunci cand bila de misca mai repede 
