...
@Component({
  selector: 'app-brickbreaker2',
  imports: [CommonModule],
  templateUrl: './brickbreaker2.html',
  styleUrl: './brickbreaker2.css'
})

export class Brickbreaker2 {
  ...
  ngOnInit() {
    ...
    //initializam arena vietilor cu 1 singura linie si 4 coloane, puse pe 0
    this.lifeArena = [];
    for (let i = 0; i < 1; i++) {
  ...
  keyEvent(event: KeyboardEvent) {
    ...
    //impiedicam scroll ul paginii cand apasam sagetile sau space
    if (
      event.code === 'ArrowUp' ||
  ...
  checkBallCollision() {
    ...
    else if(this.BallPosition.y + 1 === this.PaddlePosition.y) //verificam coliziunea cu paddle ul (pe axa y)
    {
      if(
        ...
        this.BallPosition.x === this.PaddlePosition.x + 3 //adaugam +3 pentru a putea prinde bila si cand loveste coltul din dreapta
  ...
  resetGameRound() {
    ...
    this.PaddlePosition.x = 3, this.PaddlePosition.y = 19;
  ...
  startGame() {
    ...
    this.PaddlePosition.x = 3, this.PaddlePosition.y = 19; //pozitia initiala a paddle ului
  ...
  moveBallFaster() {
    ...
    this.direction = this.checkBallCollision(); //verificam coliziunile bilei cu zidurile si paddle ul
      ...
      this.points += 2; //marim cu 2 puncte atunci cand bila de misca mai repede 
