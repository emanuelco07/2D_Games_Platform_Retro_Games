...
@Component({
  selector: 'app-brickbreaker3',
  imports: [CommonModule],
  templateUrl: './brickbreaker3.html',
  styleUrl: './brickbreaker3.css'
})

export class Brickbreaker3 {
  ...
  PaddlePosition: { x: number, y: number } = {x: 4, y: 19}; //pozitia initiala a paddle ului
  ...
  ngOnInit() {
    ...
    //inializam arena vietilor cu 1 linie si 4 coloane, puse pe 0
    this.lifeArena = [];
    for (let i = 0; i < 1; i++) {
  ...
  keyEvent(event: KeyboardEvent) {
    ...
    //impedicam scroll ul paginii cand apasam tastele folosite in joc
    if (
      event.code === 'ArrowUp' ||
  ...
  //functie pentru a verifica coliziunile bilei cu zidurile si paddle ul
  checkBallCollision() {
    ...
    else if(this.BallPosition.y + 1 === this.PaddlePosition.y) //verificam coliziunea cu paddle ul (pe axa y)
    {
      if(
        ...
        this.BallPosition.x === this.PaddlePosition.x + 2 //adaugam +2 pentru a putea prinde bila si cand loveste coltul din dreapta
  ...
  resetGameRound() {
    ...
    this.PaddlePosition.x = 4, this.PaddlePosition.y = 19;
  ...
  //functie pentru a verifica coliziunile bilei cu brick urile
  checkWall() {
  ...
  //functie pentru a muta bila in diaogonala dreapta sus (nord est)
  moveBallNorthEast() {
    this.BallPosition.x++;
    this.BallPosition.y--;
  }

  //functie pentru a muta bila in diaogonala stanga sus (nord vest)
  moveBallNorthWest() {
    this.BallPosition.x--;
    this.BallPosition.y--;
  }

  //functie pentru a muta bila in diaogonala dreapta jos (sud est)
  moveBallSouthEast() {
    this.BallPosition.x++;
    this.BallPosition.y++;
  }

  //functie pentru a muta bila in diaogonala stanga jos (sud vest)
  moveBallSouthWest() {
    this.BallPosition.x--;
    this.BallPosition.y++;
  }
  ...
  startGame() {
    ...
    this.PaddlePosition.x = 4, this.PaddlePosition.y = 19; //pozitia initiala a paddle ului
    this.BricksPosition.x = 0, this.BricksPosition.y = 0; //pozitia initiala a brick urilor
  ...
  //functie pentru a deloga utilizatorul si al trimite la pagina de login
  onLogout() {
  ...
  //functie pentru a verifica daca jocul s a castigat
  gameWon() {
